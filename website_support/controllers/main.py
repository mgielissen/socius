...
class SupportTicketController(http.Controller):
    ...
    def support_process_ticket(self, **kwargs):
        ...
        if "subcategory" in values:
            ...
            sub_category = ""
        create_dict = {'person_name':values['person_name'], 'category_id': int(values['category']), 'sub_category_id': sub_category, 'email':values['email'], 'description':values['description'], 'subject':values['subject']}
        if http.request.env.user.name != "Public user":
