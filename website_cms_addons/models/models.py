...
class WebsiteCmsMenu(models.TransientModel):
...
class event_config_settings(models.Model):
    ...
    def get_default_root_cms_page_id(self):
        """
        Getting default config values
        :param cr:
        :param uid:
        ...
        :return:
        """
        res = super(event_config_settings, self).get_values()
        ir_values = self.env['ir.values']
        root_cms_page_id = ir_values.get_default('event.settings', 'root_cms_page_id')
        return {
            'root_cms_page_id': root_cms_page_id,
        }


    def set_default_root_cms_page_id(self):
        """
        setting default values
        :param cr:
        :param uid:
        ...
        if wizard.root_cms_page_id:
            ...
            ir_values.set_default('event.settings', 'root_cms_page_id', root_cms_page_id.id)
        else:
            root_cms_page_id = False
            ir_values.set_default('event.settings', 'root_cms_page_id', root_cms_page_id)
