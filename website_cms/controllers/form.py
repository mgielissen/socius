...
class PageFormMixin(ContextAwareMixin):
    ...
    def get_template(self, main_object, **kw):
        ...
        template = super(PageFormMixin, self).get_template(main_object)
        return template
...
class CreatePage(http.Controller, PageFormMixin):
    ...
    @http.route([
        '/cms/add-page',
        '/cms/<model("cms.page"):parent>/add-page',
        '/cms/<path:path>/<model("cms.page"):parent>/add-page',
    ], type='http', auth='user', methods=['GET', 'POST'], website=True)
    def add(self, parent=None, **kw):
        """Handle page add view and form submit."""
        if request.httprequest.method == 'GET':

            template = self.get_template(parent, **kw)
            vals = self.get_render_values(parent, **kw)
            del vals['main_object']
...
class EditPage(http.Controller, PageFormMixin):
    ...
    def edit(self, main_object, **kw):
        ...
        if request.httprequest.method == 'GET':
            template = self.get_template(main_object, **kw)
            vals = self.get_render_values(main_object, **kw)
            # del vals['main_object']
            return request.render(
                template, vals
            )
