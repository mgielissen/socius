...
from odoo.addons.website_cms.utils import IMAGE_TYPES
from odoo.addons.website_cms.utils import VIDEO_TYPES
from odoo.addons.website_cms.utils import AUDIO_TYPES
from odoo.addons.website_cms.utils import download_image_from_url
from odoo.addons.website_cms.utils import guess_mimetype
from odoo.addons.website_cms.utils import AttrDict
...
def to_embed_url(url, provider='youtube'):
    ...
    qstring = urlparse3.parse_qs(parsed.query)
...
class CMSMedia(models.Model):
        ...
        for item in self:
            ...
            if not url:
                url = url_pattern.format(
                    ...
                    ob_id=item.id,
                    field_name='datas',
                    filename=item.store_fname or 'download',
                )
    ...
    def _preload_preview_image(self):
        ...
        for item in self:
            ...
            values = {}
            for fname in ('store_fname', 'url', 'datas'):
