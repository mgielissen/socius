...
class WebsiteSupportTicketInheritTimesheets(models.Model):
    ...
    def invoice_client(self):
        self.ensure_one()
        print("inside invoice client", self.subject.encode("UTF-8"))
        invoiced_state = self.env['ir.model.data'].sudo().get_object('website_support_timesheets',
                                                                     'website_ticket_state_invoiced')
        print("invoice sate", invoiced_state)
        ...
        new_invoice = self.env['account.move'].create(
            {'name': '', 'type': 'out_invoice', 'partner_id': self.partner_id.id,
             'narration': 'Support Ticket #' + str(self.id) + " " + self.subject})
        ...
        for timesheet in self.timesheet_ids:
            ...
            new_invoice.invoice_line_ids.create(
                {'invoice_id': new_invoice.id, 'name': 'Support Ticket Service (' + time_string + ')',
                 'account_id': invoice_account, 'price_unit': '0'})
...
class WebsiteSupportTicketTimesheet(models.Model):
    ...
    ticket_number_display = fields.Char(related="wst_id.ticket_number", string="Support Ticket Number")
    state = fields.Many2one('website.support.ticket.state', readonly=True, related="wst_id.state_id", string="State")
